...
try {
    ...
    CONCAT(cast(ROUND(temperatura, 0) as int), '°C') AS temperatura, 
    porcentaje_lluvia, cast(indice_UV as DECIMAL(9, 1)) as indice_UV, 
    calidad_aire, fecha, CONCAT(DATEPART(HOUR, fecha), ':00') AS hora 
    FROM clima WHERE FORMAT(fecha, 'yyyy-MM-dd') = format(dateadd(hour, -5, GETDATE()), 'yyyy-MM-dd') 
        AND FORMAT(fecha, 'yyyy-MM-dd HH') >= format(dateadd(hour, -5, GETDATE()), 'yyyy-MM-dd HH')");
